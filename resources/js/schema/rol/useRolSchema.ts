import * as z from "zod"
import { useForm } from "vee-validate"
import { toTypedSchema } from "@vee-validate/zod"

const formSchemaRolCreated = toTypedSchema(
    z.object({
        name: z.string({ message: 'Campo requerido' })
            .min(2, 'Este campo debe contener mas de 2 digito')
            .max(50, 'Este campo permite maximo 50 digito'),
    })

)

const formSchemaRolEdit = toTypedSchema(
    z.object({
        id: z.number(),
        name: z.string({ message: 'Campo requerido' })
            .min(2, 'Este campo debe contener mas de 2 digito')
            .max(50, 'Este campo permite maximo 50 digito'),
    })
)

const useRolFormcreated = () => {
    const { isFieldDirty, handleSubmit } = useForm({
        validationSchema: formSchemaRolCreated,
    })

    return { isFieldDirty, handleSubmit }
}

const useRolFormEdit = () => {
    const { isFieldDirty, handleSubmit, setValues } = useForm({
        validationSchema: formSchemaRolEdit,
        initialValues: {
            id: 0,
            name: '',
        }
    })

    return { isFieldDirty, handleSubmit, setValues }
}

export {
    useRolFormcreated,
    useRolFormEdit
}